...
class SystemNotificationPage(MainFunc):
    ...
    def write_in_doc(self, npa_id: Union[int, str], field_name: str, regulation_type: str, date: str):
        ...
        npa_id_message = f"ID проекта на сайте regulation.gov.ru: {npa_id}"
        ...
        if field_name == SettingsValues.SystemNotification.REGULATION_DISCUSS_NPA:
            ...
            main_text = "Тема сообщения: «Не поступила заявка на размещение сводки предложений».\nТекст сообщения: В 21 департамент не поступила заявка на размещение сводки предложений по"
            message_text = {
                RegulationFields.REGULATION_TYPE_851_1: f"{main_text} уведомлению о подготовке проекта НПА. Необходимо направить письмо для разработчика проекта. {npa_id_message}",
                RegulationFields.REGULATION_TYPE_1318_2: f"{main_text} уведомлению о подготовке проекта НПА. Необходимо направить письмо для разработчика проекта. {npa_id_message}",
                RegulationFields.REGULATION_TYPE_851_7: f"{main_text} проекту НПА. Необходимо направить письмо для разработчика проекта. {npa_id_message}",
                RegulationFields.REGULATION_TYPE_1318_10: f"{main_text} проекту НПА и сводному отчета. Необходимо направить письмо для разработчика проекта. ID проекта правового акта: {npa_id}",
                RegulationFields.REGULATION_TYPE_1318_11: f"{main_text} проекту решения ЕЭК. Необходимо направить письмо для разработчика проекта. {npa_id_message}",
                RegulationFields.REGULATION_TYPE_83_32: f"{main_text} отчету ОФВ. Необходимо направить письмо для разработчика отчета. {npa_id_message}",
            }
