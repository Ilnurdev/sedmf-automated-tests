...
class AuthPage(MainFunc):
    def choose_org(self):
        self.work_with_selector(*AuthPageLocators.SELECT_ORG_LOCATOR, ORG_VALUE)
    
    def choose_user(self, value):
        self.work_with_selector(*AuthPageLocators.SELECT_USR_LOCATOR, value)

    def enter_password(self):
        password = self.driver.find_element(*AuthPageLocators.ENTER_PASSWORD_LOCATOR)
        password.send_keys(CORRECT_PASSWORD)

    def click_to_enter_button(self):
        enter_button = self.driver.find_element(*AuthPageLocators.ENTER_BUTTON_LOCATOR)
        enter_button.click()
